...
Controllers.controller('appCtrl', ['$scope','$http','Apps','Data','Survey','Analytics',
	...
	$scope.appDownloaded = false;
	$scope.getApp = function(){
		if( !$scope.appDownloaded )
			Analytics.analyzePage($scope.appId + '-downloaded' );
		$scope.appDownloaded = true;
	};

	$scope.tabChanged = function( scope , index ){
		switch(index){
			case 0:
					Analytics.analyzePage($scope.appId + '-' + $scope.type + '-info');
				break;
			case 1:
					Analytics.analyzePage($scope.appId + '-' + $scope.type + '-reviews');
				break;
			case 2:
					Analytics.analyzePage($scope.appId + '-' + $scope.type + '-visualize');
				break;
		}
	};
